...
void Visualizer::initialize()
{
	...
	if (!m_font.loadFromFile(m_font_filename))
	{
...
void Visualizer::handle_key_pressed(sf::Keyboard::Key code)
{
	...
	case sf::Keyboard::Add:
        m_view.scale(1.0f / m_scale_factor);
        break;
    case sf::Keyboard::Subtract:
        m_view.scale(m_scale_factor);
        break;
...
void Visualizer::render()
{
    ...
    if (m_need_redraw) 
	{
		for (auto y = 0U; y < m_calculator.height(); ++y)
		{
			...
			{
				const auto iterations = field[x][y];

				if (iterations == m_calculator.max_iterations())
				{
					m_image.setPixel(x, m_calculator.height() - y - 1, sf::Color::Black);
				}
				else
				{
					m_image.setPixel(x, m_calculator.height() - y - 1, m_colors[static_cast < std::size_t > (
...
void Visualizer::save_pic()
{
	if(m_image.saveToFile(m_pic_filename))
	{
		std::cout << "Image saved to " << m_pic_filename << std::endl;
	}
	else
	{
		std::cerr << "Unable to save image to " << m_pic_filename << std::endl;
